...
function createSphere(gl, radius)
{
    ...
    for (i = 5; i <= 55; i++)
    {
      verts.push(radius * Math.cos((i * 2 * Math.PI)/60), radius * Math.sin((i * 2 * Math.PI)/60), 0);
    }

    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);
    var sphere = {buffer:vertexBuffer, vertSize:3, nVerts:50, primtype:gl.TRIANGLE_FAN};
    return sphere;
}
